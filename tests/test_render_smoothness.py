...
def _dense_engine(n=64, scatter=0.06, n_species=4, seed=0):
    ...
    # repro=0 у растений выключает экономику роста: этот тест про поток
    # кадров и стоимость scipy.label, а не про то, как быстро растёт мох.
    # С экономикой шаг стоит на четверть дороже, поколений за те же 3 секунды
    # проходит меньше, и сторожевая проверка «кадры вообще идут» срывалась,
    # хотя измеряемое свойство (нет стопоров) не менялось.
    from life_cube.config import DEFAULT_GENOMES, IDX
    genomes = DEFAULT_GENOMES.copy()
    genomes[:6, IDX["repro"]] = 0.0
    genomes[:6, IDX["metabolism"]] = 0.0
    genomes[:6, IDX["lifespan"]] = 0.0
    cfg = Config(n=n, seed_density=0.05, animal_share=0.3, genomes=genomes)
...
def test_component_recompute_does_not_stall_frames():
    ...
    asyncio.run(run())
    # Сторожевая проверка «кадры вообще идут», а не измеряемое свойство.
    # Порог был завязан на скорость машины (за 3 с при fps=25 кадров тем
    # больше, чем быстрее считается поколение) и срывался на загруженной
    # машине САМ ПО СЕБЕ, без единой правки в коде: замерено 18 кадров на
    # коде до экономики роста и 19 на коде после. Смысл проверки — поймать
    # «поток кадров умер совсем», поэтому порог заведомо низкий; настоящее
    # утверждение теста — отсутствие стопоров ниже.
    assert len(sent) >= 8, f"кадров почти не было — {len(sent)}, сломали не то"
